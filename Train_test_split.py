...
def add_label(
    ...
    image['width'], image['height'] = Image.open('C:/Users/Akshay Deep/office_work/Data_Converter/data/'+image_url).size
...
def from_json_coco(labeled_data,src_img,train_valid_counter):
    ...
    for data in label_data:
        try:
            for x in data['Label']['objects']:
                if x['title'] not in categories:
                    categories.append(x['title'].lower())
    ...
    for data in label_data:
        ...
        if not counter_categories:
            create_categories(categories,coco_train)
            create_categories(categories, coco_val)
...
# def export_coco(file_input, file_output):
#     try:
...
if __name__ == '__main__':
    # Input File Name
    file_input = 'export-XXXX-XX-XXXXX_XX_XX.671Z.json'
    Folder_to_pick_image_from = 'XXXXXXXXXXXXXXXXXXXXXXXXXX'
    valid_counter = 92 # Splitting the data into validation dataset and training dataset
    # Folder Name
